...
async def check_harmful_keywords(query: str, context_variables: ContextVariables) -> ReplyResult:
...
async def validate_company_name(query: str, context_variables: ContextVariables) -> ReplyResult:
...
async def check_terminate(message: str, terminate_keyword: str = "TERMINATE") -> bool:
    """
...
async def fetch_facebook_comments(company: str, context_variables: ContextVariables):
...
async def fetch_instagram_comments(company: str, context_variables: ContextVariables):
    output = f"Instagram comments for {company}: ['Love it!', 'Could be better.']"
    context_variables.set("ig_comments", output)
    return ReplyResult(message=output, context_variables=context_variables)

async def fetch_reddit_comments(company: str, context_variables: ContextVariables):
...
async def summarize_comments(fb: str, ig: str, reddit: str, context_variables: ContextVariables):
...
async def clarify_summary(summary: str, query_to_user: str, context_variables: ContextVariables):
    user_comment = input(query_to_user)
    print(user_comment)
    return ReplyResult(message=user_comment, context_variables=context_variables)
...
async def fetch_india_today_news(company: str, context_variables: ContextVariables) -> ReplyResult:
...
async def fetch_times_of_india_news(company: str, context_variables: ContextVariables) -> ReplyResult:
...
async def summarize_news(it_news: str, toi_news: str, context_variables: ContextVariables) -> ReplyResult:
...
async def extract_consumer_revenue(company: str, context_variables: ContextVariables) -> ReplyResult:
    output = f"Consumer revenue over 3 years for {company}: [100M, 110M, 120M]"
    context_variables.set("consumer_revenue", output)
    return ReplyResult(message=output, context_variables=context_variables)


async def extract_business_revenue(company: str, context_variables: ContextVariables) -> ReplyResult:
    output = f"Business revenue over 3 years for {company}: [200M, 210M, 230M]"
    context_variables.set("business_revenue", output)
    return ReplyResult(message=output, context_variables=context_variables)


async def analyze_revenue_trend(consumer_rev: str, business_rev: str, context_variables: ContextVariables) -> ReplyResult:
...
async def outline_insights(data: Dict[str, Any], context_variables: ContextVariables) -> ReplyResult:
...
async def generate_insights(outline: str, context_variables: ContextVariables) -> ReplyResult:
